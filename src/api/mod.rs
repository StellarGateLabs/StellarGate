...
fn api_v1(
    ...
    axum::Router::new().nest("/merchants", merchants).nest(
        ...
        axum::Router::new()
            ...
            .merge(redeliver)
            /* The summary aggregates one merchant's payments, so it needs the
            same credential check as the list it summarises. Unauthenticated it
            reaches the handler with no merchant identity and answers 500. */
            .route(
                "/summary",
                get(payments::summary).route_layer(middleware::from_fn_with_state(
                    state.clone(),
                    auth_middleware,
                )),
            )
            /* `get_by_id` handles its own credential check, because it serves
            both authenticated and anonymous callers — a payer confirming
            their own payment has no API key. See `payments::get_by_id`. */
